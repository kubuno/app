#!/usr/bin/env bash
# ─────────────────────────────────────────────────────────────────────────────
# build_deb.sh — construit le paquet Debian du module Kubuno App (no-code).
#
# Layout d'installation (identique aux autres modules) :
#   /usr/lib/kubuno/modules/app/{kubuno-app, module.toml, frontend/}
#   /usr/share/kubuno/modules/app/migrations/*.sql
#   /etc/kubuno/modules/app/config.toml.example
#
# Usage :
...
set -euo pipefail
cd "$(dirname "$0")"
export SQLX_OFFLINE=true   # pas de DB au build (requêtes runtime, pas de macros)
...
Description: Kubuno App — plateforme no-code visuelle façon Bubble (build ${BUILD_NUM})
EOF
