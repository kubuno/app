...
use clap::Parser;
use kubuno_app::{config::Settings, router, state::AppState};
use reqwest::Client;
...
struct Manifest {
    ...
    /// Declarative instance settings (e.g. public-publishing policy).
    #[serde(default)]
...
#[derive(Parser, Debug)]
#[command(name = "kubuno-app", version, about = "Module app (no-code) Kubuno")]
struct Cli {
    #[arg(short, long, env = "KA_CONFIG_FILE")]
...
async fn main() -> Result<()> {
    ...
    match settings.logging.format {
        kubuno_app::config::LogFormat::Json   => subscriber.json().init(),
        kubuno_app::config::LogFormat::Pretty => subscriber.init(),
    }

    tracing::info!("Kubuno App v{} démarrage…", env!("CARGO_PKG_VERSION"));

    // Sécurité : interdire toute exécution de processus sur l'hôte (voir kubuno-seccomp).
    kubuno_seccomp::lock_down_process_execution("app");
    ...
    // Migrations (search_path app,public)
    if settings.database.run_migrations {
        sqlx::query("CREATE SCHEMA IF NOT EXISTS app")
            .execute(&pool)
            .await
            .context("Création du schéma app")?;
    ...
    let files_client = Arc::new(kubuno_app::files_client::FilesClient::new(
    ...
    // first publish/create requests already see the administrator's policy.
    let instance = Arc::new(std::sync::RwLock::new(
        kubuno_app::config::instance::InstanceConfig::default(),
    ));
    if let Some(cfg) = kubuno_app::config::instance::fetch(
    ...
    register_with_core(&http, &settings).await;
    {
        ...
        tokio::spawn(async move {
            loop {
                ...
                let url = format!("{}/internal/modules/app/heartbeat", settings2.core.url);
    ...
    tracing::info!("Kubuno App démarré sur http://{addr}");
...
async fn register_with_core(http: &Client, settings: &Settings) {
    ...
    let display_name = manifest.as_ref().map(|m| m.module.display_name.as_str()).unwrap_or("App").to_string();
    ...
    let sidebar_items: Vec<Value> = manifest.as_ref()
        ...
        .unwrap_or_else(|| vec![
            json!({ "id": "app", "label": "App", "icon": "AppWindow", "path": "/app", "position": 62, "protected_folder": "App" }),
        ]);
    ...
    for attempt in 1u32.. {
        ...
        match http.post(&url).header("X-Internal-Secret", secret.as_str()).json(&payload).send().await {
            Ok(resp) if resp.status().is_success() => {
                tracing::info!("Module app enregistré auprès du core");
                return;
            }
