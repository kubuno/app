...
![The visual no-code app builder](.github/screenshots/app-list.png)

<sub>The visual no-code app builder</sub>
...
- **Office-style chrome** — the builder shares the Kubuno Office shell: a ribbon whose groups collapse responsively into dropdown chips when space runs out, a "File" backstage for opening and creating apps, and clear error dialogs when a file cannot be opened.
...
- **Administered by policy** — instance administrators control public publishing, whether published apps require a signed-in account, anonymous data writes, and per-user app / per-app record / definition-size quotas, all from the admin console (Modules ▸ App).
...
Rust 2021 · Axum 0.7 · Tokio · SQLx 0.9 via `kubuno-db` (PostgreSQL 16 · MySQL/MariaDB · SQLite) — React 19 · TypeScript · Vite · Tailwind CSS v4 · Zustand · React Query.
